...
{
    private const string _cssToggleSwitchContainer = "form-check form-switch";
    private readonly string _innerToggleId = Guid.NewGuid().ToString("N");

    public ToggleSwitch() => Size = Size.None;
    ...
    protected override string? CalculateCssClasses() =>
        new CssBuilder(_cssToggleSwitchContainer).AddClass(ToggleSizeMetadata.GetSizeCss(Size))
